...
import {
  MapPin, AlertTriangle, ArrowRight, Clock, FileText,
  CheckCircle, AlertOctagon, XCircle, RefreshCw,
} from 'lucide-react';
...
/* ------------------------------------------------------------------
   State sample data — used as defaults + fallback when API is down
   ------------------------------------------------------------------ */
const STATE_DATA = {
  Lagos: {
    name: 'Lagos',
...
/* ------------------------------------------------------------------
   All 36 States + FCT — SVG polygon paths for the choropleth map
   ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------
   Alert level → fill color mapping
   ------------------------------------------------------------------ */
const ALERT_COLORS = {
...
function getAlertColor(alertLevel) {
  return ALERT_COLORS[alertLevel] || '#CBD5E1';
}
...
export default function BacklogMapPage() {
  ...
  /* ----------------------------------------------------------------
     Fetch from GET /public/backlog-map — merges with defaults
     ---------------------------------------------------------------- */
  useEffect(() => {
    let cancelled = false;
    ...
    async function fetchMapData() {
      ...
      try {
        ...
        if (!cancelled && res && Array.isArray(res) && res.length > 0) {
          ...
          res.forEach((item) => {
            const key = item.state || item.court;
            if (key && merged[key]) {
              merged[key] = {
                ...merged[key],
                totalCases: item.activeCases ?? item.totalCases ?? merged[key].totalCases,
                avgWaitDays: item.avgWaitDays ?? merged[key].avgWaitDays,
              };
            }
          });
          setMapData(merged);
        ...
        // Graceful fallback — show default sample data rather than a dead end
        if (!cancelled) {
  ...
  /* ----------------------------------------------------------------
     Derived: active state data for the side panel
     ---------------------------------------------------------------- */
  ...
  const hasTrackingData = !!mapData[selectedState];
  const AlertIcon = activeData.alertLevel ? ALERT_ICONS[activeData.alertLevel] : null;
  ...
  function handleStateSelect(stateId) {
    ...
    if (!mapData[stateId]) {
      toast.info(`No active tracking dataset registered for ${stateId} yet.`);
    }
  }

  function handleViewCases() {
    if (activeData.caseHashId) {
      navigate(`/lookup/${encodeURIComponent(activeData.caseHashId)}`);
    } else {
      navigate('/lookup');
    }
  }
  ...
  /* ================================================================
     LOADING STATE — skeleton placeholders shaped like final content
     ================================================================ */
  if (loading) {
    return (
  ...
  /* ================================================================
     ERROR STATE — EmptyState with retry action (never a dead end)
     ================================================================ */
  if (error) {
    return (
      <div className="backlog-map-page">
        <div className="container backlog-map-container">
          <Card padding="xl" className="backlog-map-error-card">
            <EmptyState
              ...
              onAction={() => { setSimulateError(false); }}
            />
          </Card>
        </div>
      </div>
      ...
      <div className="container backlog-map-container">
        ...
        <div className="backlog-map-header">
          ...
          >
            ...
            <span>{simulateError ? 'Error Active' : 'Simulate Error'}</span>
          </button>
        </div>

        {/* --------------------------------------------------------
            2-Column Responsive Layout:
            Left  → Interactive SVG Map + Legend
            Right → Side Panel (State Stat Card)
            -------------------------------------------------------- */}
        <div className="backlog-map-grid">
          ...
          <div className="backlog-map-map-col">
            <Card padding="lg" className="backlog-map-card map-card">
              ...
              {/* Mobile State Selector — all 37 states */}
              <div className="backlog-map-mobile-select-row">
                <label
                  htmlFor="state-mobile-select"
                  className="backlog-map-mobile-label"
                >
                  Select State:
                </label>
                <select
                ...
                >
                  {/* Tracked states first */}
                  <optgroup label="Tracked States">
                    {Object.keys(STATE_DATA).map((st) => (
                      <option key={st} value={st}>
                        {st} ({STATE_DATA[st].totalCases} cases — {STATE_DATA[st].alertLevel})
                      </option>
                    ))}
                  </optgroup>
                  {/* All other states */}
              ...
              <div className="backlog-map-svg-wrapper">
                <svg
                  ...
                  aria-label="Interactive choropleth map of Nigerian states showing backlog severity"
                  role="img"
                >
                  <defs>
                    ...
                    return (
                      <g key={st.id}>
                        <path
                          ...
                          tabIndex={0}
                          role="button"
                          aria-label={`${st.name}: ${stData ? `${stData.totalCases} cases, ${stData.alertLevel} severity` : 'No data available'}`}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' || e.key === ' ') {
                              e.preventDefault();
                              handleStateSelect(st.id);
                            }
                          }}
                        />
                        <text
                ...
                {/* Hover Tooltip */}
                {hoveredState && (
                  <div className="backlog-map-tooltip" role="tooltip">
              ...
              {/* Legend — four tiers labeled exactly per spec */}
              <div
                className="backlog-map-legend"
                role="region"
                aria-label="Map severity legend"
              >
                <span className="backlog-map-legend-title">
                  Backlog Severity Legend:
                </span>
                <div className="backlog-map-legend-items">
                  ...
                  <div className="backlog-map-legend-item">
                    ...
                    <AlertOctagon size={14} color="#F97316" aria-hidden="true" />
              ...
              >
                ...
                <div className="stat-panel-header">
                  <div className="stat-panel-title-group">
                    <MapPin
                      size={20}
                      className="stat-panel-pin-icon"
                      aria-hidden="true"
                    />
                    <h3 className="stat-panel-state-name">
                      {activeData.name}
                    </h3>
                  </div>
                  ...
                  {!activeData.alertLevel && (
                    <span className="stat-panel-no-data-badge">
                      No Data
                    </span>
                  )}
                </div>
                ...
                <div className="stat-panel-metrics">
                  ...
                  <div className="stat-metric-box">
                    ...
                    <div className="stat-metric-content">
                      ...
                      <span className="stat-metric-value">
                        {hasTrackingData ? activeData.totalCases : '—'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="stat-metric-box">
                    ...
                    <div className="stat-metric-content">
                      ...
                      <span className="stat-metric-value">
                        {hasTrackingData && typeof activeData.avgWaitDays === 'number'
                  ...
                  <div className="stat-metric-box">
                    ...
                    <div className="stat-metric-content">
                      ...
                      <span className="stat-metric-value stat-metric-value--reason">
                        {hasTrackingData ? activeData.topStallReason : 'No data available'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {/* Court info */}
                {hasTrackingData && (
                  <div className="stat-panel-court">
                    <MapPin size={14} aria-hidden="true" />
                    <span>{activeData.court}</span>
                  </div>
                )}
              </div>

              {/* Action Link: View [State] Cases — navigates to public lookup */}
